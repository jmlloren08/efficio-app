...
const CreateAccomplishment = ({ labels, data, setData, post, errors, processing, handleSubmit, handleChange, fileInputRef }) => {

    return (
        <div>
            <h1 className='font-medium text-gray-500 border-b border-gray-200 dark:border-gray-700'>This week</h1>
            <form onSubmit={handleSubmit} className='mb-4'>
                <div className='flex items-center justify-end'>
                    <button
                        type="submit"
                        className={`px-4 py-2 text-sm font-semibold mt-4 bg-blue-500 text-white rounded hover:bg-blue-700 ${processing && 'opacity-50 cursor-not-allowed'}`}
                        disabled={processing}
                    >
                        {processing ? (
                ...
                <div className='overflow-y-auto max-h-full space-y-4'>
                    ...
                    <div className='flex gap-2'>
                        <div className='mb-4'>
                            <label>Planned Hours Worked <span className='text-red-500' title='Required'>*</span></label>
                            <input
                                type='number'
                                name='planned_hours_worked'
                                value={data.planned_hours_worked || ''}
                                onChange={handleChange}
                                className='block w-full mt-1 rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-300'
                                required
                            />
                            <InputError message={data.errors?.planned_hours_worked} className='mt-2' />
                        </div>
                        <div className='mb-2'>
                            <label>Actual Hours Worked <span className='text-red-500' title='Required'>*</span></label>
                            <input
                                type='number'
                                name='actual_hours_worked'
                                value={data.actual_hours_worked || ''}
                                onChange={handleChange}
                                className='block w-full mt-1 rounded-md shadow-sm dark:bg-gray-700 dark:text-gray-300'
                                required
                            />
                            <InputError message={data.errors?.actual_hours_worked} className='mt-2' />
                        </div>
                    </div>
                    <div className='flex items-center space-x-2'>
                        <label className='mr-2 font-semibold'>Do you need any help?</label>
                        <div className='flex items-center hover:cursor-pointer' onClick={() => setData('need_help', '1')}>
                            <input
                                type='radio'
                                className='mr-1 hover:cursor-pointer'
                                name='need_help'
                                value='1'
                                checked={data.need_help === '1'}
                                onChange={handleChange}
                            />
                            <span>Yes</span>
                        </div>
                        <div className='flex items-center hover:cursor-pointer' onClick={() => setData('need_help', '0')}>
                            <input
                                type='radio'
                                className='mr-1 hover:cursor-pointer'
                                name='need_help'
                                value='0'
                                checked={data.need_help === '0'}
                                onChange={handleChange}
                            />
                            <span>No</span>
                        </div>
                    </div>
                    <InputError message={data.errors?.need_help} />
                </div>
                <div className='mt-4'>
